...
import { pgTable, integer, serial, varchar, real, index, boolean } from 'drizzle-orm/pg-core';
import { cities, researchers, coordinates } from './public_schema';

export const edition = pgTable('cyclist_profile.edition', {
...
export const categories = pgTable('cyclist_profile.categories', {
  id: serial('id').primaryKey(),
  type: varchar('type'),
  name: varchar('name'),
}, (categories) => ({
  type_name_idx: index('type_name_idx').onOnly(categories.type, categories.name),
}));
...
export const editionSummary = pgTable('cyclist_profile.edition_summary', {
  editionId: integer('edition_id').references(() => edition.id).notNull(),
  summaryId: integer('summary_id').references(() => summary.id).notNull(),
});
...
export const formData = pgTable('cyclist_profile.form_data', {
  ...
  weekUsageTotalCatId: integer('week_usage_total_cat_id').references(() => categories.id),
  ...
  transportationIntegrationId: integer('transportation_integration_id').references(() => categories.id),
  transportationBeforeBicycleId: integer('transportation_before_bicycle_id').references(() => categories.id),
  distanceTimeId: integer('distance_time_id'),
  distanceTimeCategoryId: integer('distance_time_category_id').references(() => categories.id),
  yearsUsingId: integer('years_using_id').references(() => categories.id),
  sharedBicycle: boolean('shared_bicycle'),
  collisionsLastYears: integer('collisions_last_years').references(() => categories.id),
  ...
  motivationToStart: integer('motivation_to_start').references(() => categories.id),
  motivationToContinue: integer('motivation_to_continue').references(() => categories.id),
...
export const editionFormData = pgTable('cyclist_profile.edition_form_data', {
  editionId: integer('edition_id').references(() => edition.id).notNull(),
  formDataId: integer('form_data_id').references(() => formData.id).notNull(),
});

export const researcherFormData = pgTable('cyclist_profile.researcher_form_data', {
  formDataId: integer('form_data_id').references(() => formData.id).notNull(),
