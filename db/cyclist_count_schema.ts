...
export const researcherSession = pgTable('cyclist_count.researcher_session', {
  countSessionId: integer('count_session_id').references(() => session.id).notNull(),
...
export const characteristicsCount = pgTable('cyclist_count.characteristics_count', {
  id: integer('id').primaryKey(),
  sessionId: integer('session_id').references(() => session.id),
  characteristicsId: integer('characteristics_id').references(() => characteristics.id),
  count: integer('count'),
});
